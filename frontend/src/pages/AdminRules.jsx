...
function normalizeRule(rule) {
  ...
  return { ...blankDefinition(), ...definition, ruleId: rule.ruleId, ruleCode: rule.ruleCode, ruleNumber: rule.ruleNumber, subclause: rule.subclause || definition.subclause || "", title: rule.title, description: rule.description, category: rule.category, defaultSeverity: rule.defaultSeverity, enabled: rule.enabled };
}
...
export default function AdminRules() {
  ...
  const [versionsText, setVersionsText] = useState(pretty(blankDefinition().versions));
  const [message, setMessage] = useState("");
  const [busy, setBusy] = useState(false);

  async function load() {
    setMessage("");
    const response = await apiFetch(`${API_URL}/admin/rules`);
  ...
  function startNew() {
    ...
    setSelectedId(""); setDraft(value); setVersionsText(pretty(value.versions)); setShowEditor(true); setMessage("");
  }

  function startEdit(rule) {
    const value = normalizeRule(rule);
    setSelectedId(rule.id); setDraft(value); setVersionsText(pretty(value.versions)); setShowEditor(true); setMessage("");
  }
  ...
  async function save(event) {
    ...
    try {
      ...
      for (const version of versions) {
        if (!Array.isArray(version.conditions)) throw new Error(`Version ${version.version} must contain a conditions array.`);
        if (!Array.isArray(version.legalSources)) throw new Error(`Version ${version.version} must contain a legalSources array.`);
        if (!version.applicabilityCriteria || typeof version.applicabilityCriteria !== "object") throw new Error(`Version ${version.version} must contain applicabilityCriteria.`);
      }
      const definition = { ...draft, versions, enabled: Boolean(draft.enabled) };
      const payload = { ...definition, definition };
      const url = selectedId ? `${API_URL}/admin/rules/${selectedId}` : `${API_URL}/admin/rules`;
      const response = await apiFetch(url, { method: selectedId ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "Unable to save rule");
      setMessage("Rule saved. The Rules Engine will use the updated active definition on the next evaluation.");
  ...
  async function disableRule(rule) {
    if (!window.confirm(`Disable ${rule.ruleCode}? Historical inspections will remain intact.`)) return;
    setBusy(true); setMessage("");
    try {
      const response = await apiFetch(`${API_URL}/admin/rules/${rule.id}`, { method: "DELETE" });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "Unable to disable rule");
      setMessage(data.message || "Rule disabled."); await load();
    } catch (e) { setMessage(e.message); }
    finally { setBusy(false); }
  }
  ...
  return <div className="admin-rules-page">
    <div className="page-header">
      ...
      <p>Manage the definitions used by the PARAKH Rules Engine. Changes are stored in PostgreSQL and picked up dynamically.</p>
    </div>
    ...
    <div className="admin-rules-layout">
      ...
      {showEditor && <form className="admin-rules-editor" onSubmit={save}>
        ...
        <div className="rule-form-grid">
          <label>Rule ID<input value={draft.ruleId} onChange={(e) => update("ruleId", e.target.value)} required /></label>
          <label>Rule code<input value={draft.ruleCode} onChange={(e) => update("ruleCode", e.target.value)} required /></label>
          <label>Rule number<input value={draft.ruleNumber} onChange={(e) => update("ruleNumber", e.target.value)} required /></label>
          <label>Sub-clause<input value={draft.subclause || ""} onChange={(e) => update("subclause", e.target.value)} /></label>
          <label>Category<select value={draft.category} onChange={(e) => update("category", e.target.value)}>{CATEGORIES.map((x) => <option key={x}>{x}</option>)}</select></label>
          <label>Severity<select value={draft.defaultSeverity} onChange={(e) => update("defaultSeverity", e.target.value)}>{SEVERITIES.map((x) => <option key={x}>{x}</option>)}</select></label>
          <label className="span-2">Title<input value={draft.title} onChange={(e) => update("title", e.target.value)} required /></label>
          <label className="span-2">Description<textarea value={draft.description} onChange={(e) => update("description", e.target.value)} required /></label>
        </div>
        ...
        <div className="editor-help">The JSON blocks below expose the same version, applicability, condition and legal-reference structures consumed by the engine. Operators currently supported by the engine: {OPERATORS.join(", ")}.</div>
