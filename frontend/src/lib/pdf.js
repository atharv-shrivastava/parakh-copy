...
function buildPages(lines, linesPerPage = 47) {
  const pages = [];
...
function makePdf(lines) {
  ...
  for (const pageLines of pages) {
    ...
    const pageId = add(`<< /Type /Page /Parent ${pagesObj} 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 ${font} 0 R >> >> /Contents ${contentId} 0 R >>`);
...
export function downloadTextPdf(filename, sections) {
  ...
  for (const section of sections) {
    ...
    lines.push("-".repeat(Math.min(86, Math.max(12, String(section.title || "").length))));
    for (const row of section.rows || []) {
      const text = `${row.label}: ${row.value ?? "Not recorded"}`;
