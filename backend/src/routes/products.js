...
function verifyProduct({ brandName, productName, netQuantity, unit, mrp }) {
  ...
  return missing.length
    ? { status: "VIOLATION", reason: `Missing declaration(s) requiring inspection: ${missing.join(", ")}.` }
    : { status: "OKAY", reason: "Automated OCR and Rules Engine assessment completed; final legal verification remains with the inspector." };
}
...
const historySelect = {
  ...
  inspections: {
    select: {
      inspectedAt: true,
      shop: { select: { id: true, name: true, address: true, city: true, state: true } },
...
router.get("/", async (req, res) => {
  try {
    ...
    const where = {
      ...
      ...(minMrp || maxMrp
        ? { mrp: { ...(minMrp ? { gte: Number(minMrp) } : {}), ...(maxMrp ? { lte: Number(maxMrp) } : {}) } }
        : {}),
      ...(shopName
        ? { inspections: { some: { shop: { name: { contains: shopName, mode: "insensitive" } } } } }
        : {}),
    };
    const data = await prisma.product.findMany({
      ...
      select: {
        ...
        inspections: {
          select: { inspectedAt: true, shop: { select: { id: true, name: true } } },
          orderBy: { inspectedAt: "desc" },
          take: 1,
        },
      },
...
router.delete("/:id", async (req, res) => {
  try {
    const product = await prisma.product.findFirst({ where: { id: req.params.id, ...visibility(req) } });
    if (!product) return res.status(404).json({ error: "Product not found" });
    await prisma.$transaction(async (tx) => {
      await tx.inspection.deleteMany({ where: { productId: product.id } });
