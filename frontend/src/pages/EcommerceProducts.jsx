...
function EcommerceProducts() {
  ...
  async function load() {
    ...
    try {
      const response = await apiFetch(`${API_URL}/categories?sourceType=ECOMMERCE`);
      const data = await response.json().catch(() => []);
      if (!response.ok) throw new Error(data?.error || "Could not load e-commerce categories");
      setCategories(Array.isArray(data) ? data : []);
    } catch (error) { setMessage(error.message || "Could not load e-commerce categories"); }
  ...
  async function addMainCategory() {
    ...
    try {
      const response = await apiFetch(`${API_URL}/categories`, {
        ...
        body: JSON.stringify({ name: categoryName, isFinal: advancedFinal && final, sourceType: "ECOMMERCE" }),
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data?.error || "Could not add e-commerce category");
      setName(""); setFinal(false); setAdvancedFinal(false); setMessage("Private e-commerce category created for this account only.");
      await load();
    } catch (error) { setMessage(error.message || "Could not add e-commerce category"); }
  ...
  return <div className="products-page ecommerce-products-page">
    <div className="page-header"><p className="eyebrow">E-COMMERCE PRODUCT DATABASE</p><h1>E-commerce Products</h1><p>Online products use the same category → subcategory → final product hierarchy as offline products, with their own E-commerce category scope.</p></div>
