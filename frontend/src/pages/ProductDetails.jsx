...
function ProductDetails() {
  ...
  useEffect(() => { apiFetch(`${API_URL}/products/${id}`).then(async (r) => { const data = await r.json(); if (!r.ok) throw new Error(data.error || "Product not found"); setProduct(data); }).catch((e) => setError(e.message)); }, [id]);
  ...
  const acceptedFindingIds = Array.isArray(stored?.complianceReview?.acceptedFindingIds)
  ...
  const visibleFindings = allFindings.filter((finding) => {
    ...
    return findingStatus !== "VIOLATION" || !acceptedFindingIds || acceptedFindingIds.has(String(finding?.findingId));
  });
  const violations = visibleFindings.filter((finding) => String(finding?.status || "").toUpperCase() === "VIOLATION");
  ...
  const status = product.complianceStatus || "NEEDS_REVIEW"; const shop = product.inspections?.[0]?.shop; const inspector = product.inspections?.[0]?.worker?.name || product.owner?.name || getUser()?.name || "Unknown"; const path = [product.category?.parent?.parent?.parent, product.category?.parent?.parent, product.category?.parent, product.category].filter(Boolean).map((x) => x.name).join(" → ");
  async function remove() { if (!window.confirm(`Delete ${product.productName}?`)) return; const r = await apiFetch(`${API_URL}/products/${id}`, { method: "DELETE" }); const data = await r.json(); if (!r.ok) return setError(data.error || "Delete failed"); navigate(`/products/category/${product.categoryId}`); }
  function downloadPdf() { downloadProductPdf({ product, user: getUser(), violations }); }
  return <div className="products-page"><Link to={`/products/category/${product.categoryId}`} className="back-link">← Back to {product.category?.name}</Link><div className="page-header"><p className="eyebrow">REGISTERED PRODUCT</p><h1>{product.productName}</h1><p>{product.brandName || "Company not recorded"}</p><p>{path}</p></div>
