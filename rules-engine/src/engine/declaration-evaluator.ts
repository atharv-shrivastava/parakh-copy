...
function result(id: string, code: string, number: string, status: EvaluationStatus, field: string, message: string, missing?: string[]): Finding {
  return {
    findingId: id, ruleId: code, ruleCode: code, ruleNumber: number, ruleVersion: 1, status, field, message,
    missingEvidence: missing, legalReferences: [SOURCE], severity: status === 'VIOLATION' ? 'CRITICAL' : 'HIGH', requiresLegalReview: false
  };
}

export function declarationFindings(r: InspectionRequest): Finding[] {
  const out: Finding[] = [];
  const declarations = (r as unknown as { declarations?: Record<string, unknown> }).declarations;
