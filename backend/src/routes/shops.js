...
router.get("/", async (req, res) => {
  try {
    ...
    const shops = await prisma.shop.findMany({
      ...
      include: {
        inspections: { include: { product: true }, orderBy: { inspectedAt: "desc" } },
      },
      orderBy: { name: "asc" },
    });
    ...
    const data = shops.map((shop) => {
      ...
      const computedStatus = statuses.includes("VIOLATION")
        ? "NON_COMPLIANT"
        : statuses.includes("NEEDS_REVIEW") || statuses.includes("UNABLE_TO_VERIFY")
          ? "REVIEW"
          : "COMPLIANT";
      return {
        id: shop.id,
        ...
        inspectionCount: shop.inspections.length,
        lastInspection: shop.inspections[0]?.inspectedAt || null,
...
router.get("/:id", async (req, res) => {
  try {
    const shop = await prisma.shop.findFirst({
      where: { id: req.params.id, ...visible(req) },
      include: {
        inspections: {
          include: {
...
router.get("/:id/products", async (req, res) => {
  try {
    ...
    const inspections = await prisma.inspection.findMany({
      where: { shopId: shop.id, ...(req.user.role === "ADMIN" ? {} : { workerId: req.user.id }) },
      include: {
        product: {
          include: {
    ...
    console.error(error);
    res.status(500).json({ error: error?.message || "Failed to fetch shop products" });
  }
});
