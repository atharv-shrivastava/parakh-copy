...
function Reports() {
  const [products, setProducts] = useState([]); const [query, setQuery] = useState(""); const [status, setStatus] = useState("ALL"); const [brand, setBrand] = useState(""); const [shop, setShop] = useState(""); const [dateFrom, setDateFrom] = useState(""); const [dateTo, setDateTo] = useState(""); const [error, setError] = useState(""); const [loading, setLoading] = useState(true); const user = getUser();
  ...
  const shown = useMemo(() => products.filter((p) => { const inspection = p.inspections?.[0]; const text = `${p.productName || ""} ${p.brandName || ""} ${p.category?.name || ""} ${inspection?.shop?.name || ""} ${p.barcode || ""}`.toLowerCase(); return (!query.trim() || text.includes(query.trim().toLowerCase())) && (status === "ALL" || p.complianceStatus === status) && (!brand.trim() || (p.brandName || "").toLowerCase().includes(brand.trim().toLowerCase())) && (!shop.trim() || (inspection?.shop?.name || "").toLowerCase().includes(shop.trim().toLowerCase())) && (!dateFrom || new Date(inspection?.inspectedAt || p.createdAt) >= new Date(dateFrom + "T00:00:00")) && (!dateTo || new Date(inspection?.inspectedAt || p.createdAt) <= new Date(dateTo + "T23:59:59.999"))); }), [products, query, status, brand, shop, dateFrom, dateTo]);
  function clear() { setQuery(""); setStatus("ALL"); setBrand(""); setShop(""); setDateFrom(""); setDateTo(""); }
