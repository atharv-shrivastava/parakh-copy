...
function analyzePixels(data, width, height) {
  ...
  let darkPixels = 0; let edgeDarkPixels = 0;
  ...
  for (let y = 0; y < height; y += 1) {
    ...
    if ((!textLike || y === height - 1) && bandStart >= 0) {
      ...
      if (end - bandStart + 1 >= 2 && end - bandStart + 1 <= Math.max(60, height * 0.12)) lineBands.push({ top: bandStart, height: end - bandStart + 1 });
      bandStart = -1;
    }
  }
  ...
  const medianLineHeight = lineBands.length ? [...lineBands.map((x) => x.height)].sort((a, b) => a - b)[Math.floor(lineBands.length / 2)] : 0;
...
export default function ScanVisualCheck() {
  ...
  useEffect(() => {
    let stopped = false; let observer; let running = false;
    const run = async () => {
      ...
      if (!stopped) setResults(next.filter(Boolean)); running = false;
    };
    const attach = () => {
    ...
    const timer = window.setInterval(attach, 700); attach();
  ...
  const placementLabel = results.some((item) => item.placement === "REVIEW") ? "Review" : results.every((item) => item.placement === "SCREENED") ? "Screened" : "Needs review";
  ...
  return <section className="scan-visual-check">
    <div className="section-heading"><div><h2>Visual inspection</h2><p>Automatic image screening for readability, text presence, layout crowding and calibrated text-size estimation.</p></div><button type="button" className="secondary-button" onClick={() => setOpen((value) => !value)}>{open ? "Hide" : "Show"}</button></div>
    {open && <>
      ...
      <div className="visual-check-calibration">
        <label><span>Known package face width (mm)</span><input type="number" min="1" step="0.1" placeholder="Optional calibration" value={referenceWidth} onChange={(event) => setReferenceWidth(event.target.value)} /></label>
        <div><strong>Estimated text height</strong><span>{estimatedMm === null ? "Needs calibration" : `${estimatedMm.toFixed(2)} mm`}</span><small>Estimate uses detected text-line height relative to the image width. It is not a legal threshold by itself.</small></div>
      </div>
      <div className="visual-check-grid">
        {results.map((item, index) => <div className="visual-check-card" key={index}>
      ...
      <div className="visual-check-note">Declaration presence is cross-checked through the OCR result shown above. Placement is a visual screening signal, not a final legal determination. Rules with measured font thresholds should use the calibrated value and the applicable legal rule.</div>
    </>}
  </section>;
}
